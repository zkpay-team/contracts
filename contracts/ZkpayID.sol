...
contract ZkpayID is ERC2771RecipientUpgradeable, ERC721Upgradeable, UUPSUpgradeable {
    ...
    /**
     * @notice Zkpay Profile information struct
    ...
    /**
     * @notice Zkpay ID to Profile struct
     */
    mapping(uint256 => Profile) public profiles;

    /**
     * @notice Address to Zkpay id
     */
    ...
    /**
     * @notice Whether a Zkpay ID has done some activity in the protocol (created a service or proposal)
     */
    ...
    /**
     * @notice Check whether a Zkpay ID is valid.
     * @param _profileId The Zkpay ID to check
     */
     ...
     * @param _profileId The Zkpay ID of the user
     ...
     * @param _tokenId1 The Zkpay ID of the user 1
     * @param _tokenId2 The Zkpay ID of the user 2
     */
    ...
    /**
     * @notice Returns the price to mint a Zkpay ID with the given handle.
     * @param _handle Handle to check
     */
    ...
    /**
     * @notice Allows a user to mint a new ZkpayID.
     * @param _handle Handle for the user
     */
    function mint(
        string calldata _handle
    ) external payable canMint(_msgSender(), _handle) canPay(_handle) returns (uint256) {
        require(mintStatus == MintStatus.PUBLIC, "Public mint is not enabled");
        address sender = _msgSender();
        _mint(sender, nextProfileId.current());
        return _afterMint(sender, _handle, msg.value);
    }

    /**
     * @notice Allows a user to mint a new ZkpayID for another address, paying the fee.
     * @param _address Address to mint the Zkpay ID for
     * @param _handle Handle for the user
     */
    function mintForAddress(
        address _address,
        string calldata _handle
    ) external payable canMint(_address, _handle) canPay(_handle) returns (uint256) {
        require(mintStatus == MintStatus.PUBLIC, "Public mint is not enabled");
        _mint(_address, nextProfileId.current());
        return _afterMint(_address, _handle, msg.value);
    }

    /**
     * @notice Allows users who reserved a handle to mint a new ZkpayID.
     * @param _handle Handle for the user
    ...
    /**
     * @notice Allows to set whether a Zkpay ID has done some activity (created a service or proposal)
     * @param _profileId The Zkpay ID of the user
     */
    ...
    /**
     * @notice Allows the owner to mint a new ZkpayID for a user for free.
    ...
    function freeMint(
        ...
        string calldata _handle
    ) external canMint(_userAddress, _handle) onlyOwner returns (uint256) {
        _mint(_userAddress, nextProfileId.current());
        return _afterMint(_userAddress, _handle, 0);
    }
     ...
     */
    function _afterMint(address _userAddress, string memory _handle, uint256 _fee) private returns (uint256) {
        ...
        emit Mint(_userAddress, userProfileId, _handle, _fee);
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual override(ERC721Upgradeable) {
        ...
        require(balanceOf(to) == 0, "Receiver already has a Zkpay ID");
        ids[from] = 0;
        ids[to] = tokenId;
    ...
    /**
     * @notice Check if _msgSender() can pay the mint fee for a Zkpay id with the given handle
     * @param _handle Handle for the user
     */
    ...
    /**
     * Check if it is possible to mint a new ZkpayID for a given address.
     * @param _userAddress Address to mint Zkpay for.
     * @param _handle Handle for the user
     */
    modifier canMint(address _userAddress, string calldata _handle) {
        require(balanceOf(_userAddress) == 0, "You already have a ZkpayID");
    ...
    /**
     * Emit when new ZkpayID is minted.
     * @param user Address of the owner of the ZkpayID
     * @param profileId The Zkpay ID of the user
     * @param handle Handle for the user
     * @param fee Fee paid to mint the ZkpayID
     */
    event Mint(address indexed user, uint256 profileId, string handle, uint256 fee);
